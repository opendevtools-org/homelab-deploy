...
function Sync-GitIgnore {
  $upstreamPath = Join-Path $upstream ".gitignore"
  $upstreamSnapshot = Join-Path $siteRoot ".gitignore.upstream"
  $customPath = Join-Path $siteRoot ".gitignore.custom"
  $effectivePath = Join-Path $siteRoot ".gitignore"
  $placeholder = "# Site-specific ignore rules go here. Product rules are in .gitignore.upstream."

  if (-not (Test-Path $upstreamPath)) { return }

  $product = [System.IO.File]::ReadAllText($upstreamPath).TrimEnd()
  if (-not (Test-Path $customPath)) {
    $extras = @()
    if (Test-Path $effectivePath) {
      $extras = @(Get-GitIgnoreExtras ([System.IO.File]::ReadAllText($effectivePath)) $product)
    }
    $customBody = if ($extras.Count -gt 0) { ($extras -join "`n") + "`n" } else { $placeholder + "`n" }
    [System.IO.File]::WriteAllText($customPath, $customBody, [System.Text.UTF8Encoding]::new($false))
  }

  Copy-Item $upstreamPath $upstreamSnapshot -Force
  $custom = [System.IO.File]::ReadAllText($customPath).Trim()
  $header = @"
