...
On first Platform start, Hub installs **Guacamole** from the community catalog (`HUB_DEFAULT_MARKET_PLUGINS=guacamole`) unless you already uninstalled it. Plugin **backends** join Compose project `homelab-backend` via `docker-compose.apps.yml`; plugin **frontends** join `homelab-frontend` via `docker-compose.frontend.apps.yml`. They are not a separate `homelab-plugin-*` stack. Compose files live under `data/hub/plugins/<id>/`. Runtime data (`data/hub/plugins/*/data/`) is gitignored. Open Guacamole from the Hub catalog (`/p/guacamole/`). Set `HUB_DEFAULT_MARKET_PLUGINS=none` to skip.
